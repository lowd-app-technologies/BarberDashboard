...
export default function ClientLogin() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      // Indicamos que estamos na área de cliente (isClientArea = true)
      // o wrapper do firebase vai se encarregar de validar o perfil do usuário
      await login(data.email, data.password, true);
      
      // Se chegou aqui, é porque está tudo certo.
      // Redirecionamos para a página de agendamento
      setLocation('/booking');
    } catch (error: any) {
      toast({
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Entre com sua conta para agendar um serviço
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
        ...
        <CardFooter className="flex flex-col gap-4">
          <div className="text-center text-sm text-muted-foreground">
            Não tem uma conta?{" "}
            <Link href="/booking/register" className="text-primary hover:underline">
